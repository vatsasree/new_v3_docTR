...
parser.add_argument("--pathj",
                    help="Path to json",
                    type=str, default="/data3/sreevatsa/reclists/image_recall_list_202401125432.json") #6/1/24: change this to the updated file name that has datetime
parser.add_argument("--paths",
                    help="Path to save files",
                    type=str, default="/data3/sreevatsa/Datasets/dataset5_8124")
parser.add_argument("--pathg",
                    help="Path to ground truth",
                    type=str, default="/data3/sreevatsa/mergedData/split_jsons/")
...
split_direc = '/data3/sreevatsa/mergedData/split_jsons'
mergedImagesdirec = '/data3/sreevatsa/mergedData/mergedImages'
...
def make_labels(impath, labels):
    ...
    for k in image_keys:
        ...
        # print("k: ", k)
        # path = k.split('/')[1] #6/1/24: changing here since the path is different now
        path = k
...
easylist = [os.path.join(mergedImagesdirec, '_'.join([item[2].split('_')[0],item[2].split('_')[2]])) for item in all_recalls_docs if float(item[0]) > cut_off_2]
mediumlist = [os.path.join(mergedImagesdirec,'_'.join([item[2].split('_')[0],item[2].split('_')[2]])) for item in all_recalls_docs if float(
    item[0]) > cut_off_1 and float(item[0]) <= cut_off_2]
hardlist = [os.path.join(mergedImagesdirec,'_'.join([item[2].split('_')[0],item[2].split('_')[2]])) for item in all_recalls_docs if float(item[0]) <= cut_off_1]
#pt2; pt1 to pt2 are the same functionality, path is given in a variable name and that variable name is used here 
