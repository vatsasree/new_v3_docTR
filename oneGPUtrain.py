...
# easy_train_set_d_1 = CustomDetectionDataset(
...
#         ),
#         use_polygons= False,
#         custom_transform=crop_image_without_cutting_boxes2,
#     )
###########################################################################################
...
def main(args):
    ...
    # print("Reached---------------------", type(easy_train_set_d_1))
    # print("Reached---------------------", len(easy_train_set_d_1))
    # imgt,tart = easy_train_set_d_1[0]
    # print("Reached---------------------", type(imgt))
    # print("Reached---------------------", type(tart))
    # print("Reached---------------------", imgt.shape)
    ...
    #commented 665-722 for one GPU setup
    ...
    # Backbone freezing
    if args.freeze_backbone:
        model.feat_extractor.requires_grad_(True) #False->True to unfreeze all the layers

    # GPU
    device = torch.device("cuda:0")  # Use only the first GPU
    model = model.to(device)

    # Resume weights
    if isinstance(args.resume, str):
        print(f"Resuming {args.resume}")
        checkpoint = torch.load(args.resume, map_location=device)  # Load to the correct device
    ...
    #train_set_d1 -> dataset1 (Consortium) (more in number)
    #train_set_d2 -> dataset2 (New data) (less in number)
    #splitBalanced_train_set_d2 -> balanced for batch split ; dataset2 (New data) (more in number to match train_set_d1)
    ...
    mixed_sampler = MixedRandomSampler(train_set_d1,splitBalanced_train_set_d2)
    ...
    for epoch in mb:
        ...
        torch.save(model.state_dict(), f"/data3/sreevatsa/models/{exp_name}_epoch{epoch}.pt")
...
def parse_args():
    ...
    parser.add_argument("-j", "--workers", type=int, default=6, help="number of workers used for dataloading")
